...
class DataBase:
    ...
    def query_annual_report_with_auditors(self, auditors:Union[list, str] = []):
        auditors = auditors if type(auditors) is list else [auditors]
        auditors = [auditor.lower() for auditor in auditors]
        with self.Session() as session:
            query = session.query(AnnualReport).join(Auditor).filter(
                func.lower(Auditor.name).in_(auditors)) if auditors else session.query(AnnualReport).join(Auditor)
        results = query.all()
...
if __name__ == "__main__":
    ...
    df = db.query_annual_report_with_auditr_and_kam_tags(big4, tags)